...
client.on('message', msg => {
    if (msg.author.id != client.user.id) {
		if (msg.content.substring(0, 3).toLowerCase() == 'pb.'){
			var args = msg.content.substring(3).split(" ");
			switch(args[0].toLowerCase()){
			case 'create':
				if(keys.indexOf(args[1].toLowerCase()) < 0){
					msg.channel.send("Specified language code does not exist.");
					break;
				}
				if(channels.has(msg.channel)){
					msg.channel.send("This channel already has a language defined.")
					break;
				}
			...
			case 'delete':
				if(channels.delete(msg.channel)){
					...
					msg.channel.send("No translation exists here.");
				}
				break;
			}
		}else{
			channels.forEach(function(value,key,map){
				if(key === msg.channel) return	;
				var xhttp = new XMLHttpRequest();
				xhttp.onreadystatechange = function() {
					if (this.readyState == 4 && this.status == 200) {
						//console.log(JSON.parse(this.responseText));
						key.send(msg.author.tag+": "+JSON.parse(this.responseText)["text"][0]);
					}
				}
